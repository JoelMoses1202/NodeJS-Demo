/* Require fileRead and response*/
let readFile = require("../utils/fileRead");
let responseMsg = require("../utils/response");
let addControll = require("../services/buddyRead.service");
let constant=require("../utils/constant");

/**
 * This function handles an HTTP request and sends a response.
 * @param {object} req - The request object. Request object is sent to function.
 * @param {object} res - The response object. Respond object is sent to function.
 */

const displayAllBuddy = (req, res) => {
  addControll.displayAll(req, res);
};

/**
 * This function handles an HTTP request and sends a response.
 * @param {object} req - The request object. Request employee id to validate data.
 * @param {object} res - The response object. Responds the the validation error object.
 */
const displayOneBuddy = (req, res) => {
  let flag = 0;
  let val = req.params["id"];
  let users = readFile.fileRead();
  for (i = 0; i < users.length; i++) {
    if (users[i].employeeId === val + "") {
      flag = 1;
    }
  }
  if (flag == 0) {
    res.send(responseMsg(constant.ERROR, constant.NOT_VALID_EMPLOYEE_ID, 404));
    return;
  } else { 
    addControll.displayOne(req, res);
  }
};
module.exports = { displayAllBuddy, displayOneBuddy };
